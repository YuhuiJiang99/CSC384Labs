    ...
    for i in range(0, length):
        for j in range(0, length):
            ...
            if cell in PlayerDic[color]:
                ...
                # else:
                #     bonus = allianceBonus(i, j, board, color, oppColor)
                #     # print(f"we have {bonus} alliance")
                #     heur += bonus*10
...
def allianceBonus(i, j, board, color, oppColor):
    bonus = 0
    for x in [-1, 1]:
        for y in [-1,1]:
            if board[i+x][j+y] in PlayerDic[oppColor]:
                if board[i-x][j-y] in PlayerDic[color]:
                    bonus += 1
                else:
                    bonus -= 1
            elif board[i+x][j+y] in PlayerDic[color]:
                bonus += 1
    return bonus
    ...
    if nextStates is None:
        ...
        for nextState in nextStates:
            if limit > 1:
                ...
                nextValue = compute_utility(nextState, OppDic1[color])
    ...
    if nextStates is None:
        ...
        for nextState in nextStates:
            if limit > 1:
                ...
                nextValue = compute_utility(nextState, color)
...
def alphabeta_min_node(state, color, alpha, beta, limit, caching=0, ordering=0):
    ...
    nextStates = successors(state, color)
    if nextStates is None:
        stopValue = compute_heuristic(state, OppDic1[color])
        if caching:
            cache[(tup, color)] = (stopValue, None)
        return stopValue, None
    else:
        ...
        for nextState in nextStates:
            if limit > 1:
                ...
                nextValue = compute_heuristic(nextState, OppDic1[color])
            if nextValue < minValue:
                minValue, minState = nextValue, nextState 
            if nextValue <= alpha:
                break
...
def alphabeta_max_node(state, color, alpha, beta, limit, caching=0, ordering=0):
    ...
    if nextStates is None:
        stopValue = compute_heuristic(state, color)
        if caching:
            cache[(tup, color)] = (stopValue, None)
        return stopValue, None
    else:
        ...
        for nextState in nextStates:
            if limit > 1:
                ...
                nextValue = compute_heuristic(nextState, color)
            if nextValue > maxValue:
                maxValue, maxState = nextValue, nextState 
            if nextValue >= beta:
                break
