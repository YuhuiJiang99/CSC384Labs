#   Look for #IMPLEMENT tags in this file. These tags instorageMatrixate what has
...
#Global variables that will be used in the heur_alternate
preval = 0
matrix = dict()
...
def edgeDL(state, box, unpickedStorage):
    ...
    if upBlock or downBlock: #check if the box hit top or bottom wall
        return not any([box[1]==storage[1] for storage in unpickedStorage]) #if no storage is along the top/bottom wall, then there is a edge DL
    elif leftBlock or rightBlock: #check if the box hit left or right wall
        return not any([box[0]==storage[0] for storage in unpickedStorage])#if no storage is along the left/right wall, then there is a edge DL
    else:
...
def set_matrix(state):
    #Create a step-cost matrix which stores an estimated step-cost to the storage at the corner
    global matrix
    matrix.clear()
    width = state.width
    height = state.height
    corner = None
    cornerCount = 0
    for tup in [(0,0),(width-1, 0), (0, height-1), (width-1, height-1)]:
        if tup in state.storage:
            corner = tup
            cornerCount += 1
    if cornerCount == 1 and check_use_matrix(state, corner):
        for i in range(0, state.width):
            for j in range(0, state.height):
                tup = (i, j)
                minCornerDis = abs(tup[0]-corner[0]) + abs(tup[1]-corner[1])
                matrix[tup] = minCornerDis

def check_use_matrix(state, corner):
    if corner == (0,0):
        for i in range(0, 3):
            if (i, 0) not in state.storage:
                return False
        for j in range(0, 3):
            if (0, j) not in state.storage:
                return False
        return True
    elif corner == (state.width-1, 0):
        for i in range(state.width-1, state.width-4, -1):
            if (i, 0) not in state.storage:
                return False
        for j in range(0, 3):
            if (state.width-1, j) not in state.storage:
                return False
        return True
    elif corner == (state.width-1, state.height-1):
        for i in range(state.width-1, state.width-4, -1):
            if (i, state.height-1) not in state.storage:
                return False
        for j in range(state.height-1, state.height-4, -1):
            if (state.width-1, j) not in state.storage:
                return False
        return True
    elif corner == (0, state.height-1):
        for i in range(0, 3):
            if (i, state.height-1) not in state.storage:
                return False
        for j in range(state.height-1, state.height-4, -1):
...
def heur_alternate(state):
    ...
    #4. Step-cost matrix(for solving corner-block problems):
        #A matrix is built to estimate the step-cost of every cell in the board to the corner(like a Othelo matrix). The value in this matrix 
        #will be used as an additional standard to estimate the step-cost of every box location to the storage at corner, which is the deepest. 
    global preval
    global matrix
    ...
    if state.parent and state.parent.boxes == state.boxes:
        ...
        for box in state.boxes:
            if len(matrix) > 0:
                ...
                preval += matrix[box]
            if box not in state.storage:
                if cornerDL(state, box) or wallConsecDL(state, box) or edgeDL(state, box, unpickedStorages):                  
                    preval = float('inf')
                    return float('inf')
...
#19 solved, 14 outperformed in local laptop
def anytime_weighted_astar(initial_state, heur_fn, weight=1., timebound = 10):
    ...
    while curTime < endTime:
        ...
        costBound = (float('inf'), float('inf'), ret_state.gval + heur_fn(ret_state))#Update the costbound to be current solution gval
        curTime = os.times()[0] #Update current time
        if weight > 1:
            weight -= 1 #Update the weight
    return ret_state

#19 solved, 17 outperformed in local laptop
def anytime_gbfs(initial_state, heur_fn, timebound = 10):
    ...
    while curTime < endTime:
        ...
        curTime = os.times()[0] #Update current time
    # if ret_state:
    #     print(f"My cost is {ret_state.gval}")
    return ret_state
